...
const cache = process.env.NODE_ENV === 'development'
  ? global.cache ?? (global.cache = new LRU({ ...cacheOptions, maxAge: 1000 * 60 * 10 }))
 ...
 */
export async function handleCache<FetcherFunction extends () => ReturnType<FetcherFunction>>(cacheKey: string, fetchFn: FetcherFunction) {
